...
Sets your class using the class key from `osr_osr_rpg/classes` (e.g. `fighter`, `thief`, `elf`).
...
Allocate L1 expertise points for thief-type classes (d6 skill system). Each point adds +1-in-6 to a skill, up to 5-in-6 total. You must spend exactly the required number of points (usually 4 for Thief).

Use skill keys from `osr_osr_rpg/classes` (e.g. `hide_in_shadows`, `move_silently`, `climb_walls`).
