module AresMUSH
  module OsrRpg
    class AbilityCmd
      ...
      def check_pairs
        return t('osr_rpg.ability_args_required') if self.pairs.empty?
        nil
      end
      ...
      def handle
        ClassTargetFinder.with_a_character(self.name, client, enactor) do |model|
          errors = OsrRpg::Chargen.set_abilities(model, self.pairs)
          if errors.any?
            client.emit_failure errors.join('%r')
          else
            lines = self.pairs.map { |ab, val| "#{ab.upcase}=#{val}" }
            client.emit_success t('osr_rpg.abilities_set', results: lines.join(', '))
          end
        end
      end
    end
