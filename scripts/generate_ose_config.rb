...
# One-time generator for OSE game config YAML files.
require 'yaml'
...
write_yaml(File.join(CONFIG_DIR, 'ose.yml'), ose_core)
...
write_yaml(File.join(CONFIG_DIR, 'ose_classes_human.yml'), { 'ose' => { 'classes' => humans } })
...
write_yaml(File.join(CONFIG_DIR, 'ose_classes_demihuman.yml'), { 'ose' => { 'classes' => demihumans } })

write_yaml(File.join(CONFIG_DIR, 'ose_classes_supplemental.yml'), {
  'ose' => {
    'classes' => {
      'necromancer' => {
...
write_yaml(File.join(CONFIG_DIR, 'ose_spells.yml'), { 'ose' => { 'spells' => spells } })
